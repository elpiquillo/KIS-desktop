const themesColor = {
  default: 'linear-gradient(57.4deg, rgba(249,206,183,1) 4.6%, rgba(234,174,202,1) 26%, rgba(210,182,226,1) 49.1%, rgba(173,209,242,1) 86.5%)',
  lightBlue: 'linear-gradient(to right, rgba(239,249,249,1) 0%, rgba(182,199,226,1) 100.2%)',
  sunny_day: 'linear-gradient(to right, #f6d365 0%, #fda085 100%)',
  cool_blues: 'linear-gradient(to right, #a1c4fd 0%, #c2e9fb 100%)',
  purple_dreams: 'linear-gradient(to right, #9fa4c4 0%, #9e768f 100%)',
  pinky: 'linear-gradient( 110.5deg,  rgba(248,196,249,0.66) 22.8%, rgba(253,122,4,0.15) 64.6% )',
  lime_punch: 'linear-gradient(to right, #c1dfc4 0%, #deecdd 100%)',
  coral_blush: 'linear-gradient(to right, #ff9966 0%, #ff5e62 100%)',
  sunday_morning: 'linear-gradient(326deg, #861657 0%, #ffa69e 74%)',
  desert: 'linear-gradient(315deg, #f1dfd1 0%, #f6f0ea 74%)',
  darkDark: 'radial-gradient( circle farthest-corner at 10% 20%,  rgba(90,92,106,1) 0%, rgba(32,45,58,1) 81.3% )',
  blueDark: 'linear-gradient( 171.8deg,  rgba(5,111,146,1) 13.5%, rgba(6,57,84,1) 78.6% )',
  forestDark: 'linear-gradient(135deg, rgba(0,70,64,1) 0%, rgba(34,193,195,1) 100%)',
  red_mistDark: 'linear-gradient(to right, #000000 0%, #e74c3c 100%)',
  midnightPurpleDark: 'linear-gradient(135deg, rgba(30,60,114,1) 0%, rgba(42,82,152,1) 100%)',
  charcoalMidnightDark: 'linear-gradient(135deg, rgba(35,37,38,1) 0%, rgba(65,67,69,1) 100%)',
}

export default themesColor;